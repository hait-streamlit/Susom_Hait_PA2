...
# over 20 rounds, white won 100% of the time
...
def play_single_round(round_index):
    ...
    while True:
        if turn == 1:
            board, nodes, piece = Agent(boardmatrix=current_board, turn=turn, depth=3, function=2, is_alpha_beta=False, type=0).decision()
            current_board = board.getMatrix()
            b_nodes += nodes
        else:
            board, nodes, piece = Agent(boardmatrix=current_board, turn=turn, depth=5, function=1, is_alpha_beta=True, type=0).decision()
            current_board = board.getMatrix()
            w_nodes += nodes
