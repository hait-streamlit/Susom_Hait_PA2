# statistic for AB Offensive 2 vs AB Defensive 1
# black won 60% of the time over 20 rounds 
...
def play_single_round(round_index):
    ...
    while True:
        if turn == 1:
            board, nodes, piece = Agent(boardmatrix=current_board, turn=turn, depth=5, function=4, is_alpha_beta=True, type=1).decision()
            current_board = board.getMatrix()
            b_nodes += nodes
        else:
            board, nodes, piece = Agent(boardmatrix=current_board, turn=turn, depth=5, function=1, is_alpha_beta=True, type=1).decision()
            current_board = board.getMatrix()
            w_nodes += nodes
